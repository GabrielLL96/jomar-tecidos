-- Fase 2 do Melhor Envio (geracao de etiqueta real): create_order() so
-- guardava o PRECO do frete escolhido, nunca o serviceId (qual
-- transportadora/servico especifico) -- sem isso, geracao de etiqueta nao
-- sabe o que comprar.
alter table orders
  add column shipping_service_id integer;

-- URL do PDF da etiqueta (endpoint /shipment/print da Melhor Envio).
alter table deliveries
  add column melhor_envio_label_url text;

-- addresses.street e um texto livre unico ("Rua X, 123") -- a Melhor Envio
-- exige number/district como campos separados no payload de to/from.
-- Colunas novas, nullable: parsing best-effort do street livre fica como
-- fallback na edge function pra enderecos antigos sem esses campos.
