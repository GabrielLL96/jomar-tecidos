-- Status "Agendado" existia no enum/UI mas não tinha nenhum campo de data
-- por trás — escolher esse status só deixava o cupom permanentemente inválido
-- (isCouponValid rejeitava qualquer coisa != 'active'), até um admin lembrar
-- de trocar manualmente pra "Ativo" no dia certo. E o badge "Ativo"/"Esgotado"
-- na tabela era só o campo `status` cru, nunca recalculado contra
-- expires_at/used_count reais — podia mostrar "Ativo" pra um cupom já vencido
-- ou esgotado. Ver _Feedback.md.
--
-- `starts_at` é o campo real de agendamento; `status` continua existindo
-- como intenção manual do admin (ativo, ou desligado antes da hora via
-- expired/depleted) — o app agora recalcula o status "efetivo" a partir de
-- starts_at/expires_at/uso real (ver computeCouponStatus em coupon-utils.ts),
-- em vez de confiar cegamente no valor guardado.
alter table public.coupons add column starts_at timestamptz;

-- create_order(): substitui o check binário `status <> 'active'` (que
-- rejeitava 'scheduled' pra sempre, mesmo depois da data de início) por
-- checks explícitos que espelham exatamente computeCouponStatus() do
-- frontend — status expired/depleted são decisão manual do admin e
-- respeitados como tal; expires_at/max_uses/starts_at são recalculados
-- contra o real. Corpo idêntico ao de 20260813030000, só adiciona
-- starts_at — signature de tipos não muda, `create or replace` substitui de
-- verdade.
...
begin
  ...
  if p_coupon_id is not null then
    ...
    -- expired/depleted manuais são decisão explícita do admin (desligar
    -- antes da hora) — 'scheduled' nunca é gravado mais, é só calculado no
    -- frontend, então não precisa de caso especial aqui.
    if v_coupon.status = 'expired' or v_coupon.status = 'depleted' then
