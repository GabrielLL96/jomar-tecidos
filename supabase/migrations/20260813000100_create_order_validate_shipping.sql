-- Fecha o gap documentado no spec 2026-08-13-melhor-envio-fase2: create_order()
-- confiava cegamente em p_shipping_cost vindo do client (só validava >= 0).
-- Agora:
--   1. Frete grátis é decidido pelo servidor (subtotal >= threshold de
--      site_settings), sem depender do que o client mandar.
--   2. Se o client mandar uma cotação real (p_shipping_quote_id +
--      p_shipping_service_id), o preço vem de shipping_quotes (gravado pela
--      Edge Function melhor-envio-shipping-calculate), não do client.
--   3. Sem cotação (taxa fixa, produto sem peso/dimensão): ainda confia em
--      p_shipping_cost — limitação conhecida, documentada no spec (risco
--      baixo, valor fixo e conhecido).
--
-- Diferente das mudanças anteriores nesta function (que só adicionavam
-- `default` a parâmetros já existentes, mesmo signature), esta adiciona 2
-- parâmetros NOVOS — muda a lista de tipos. `create or replace function` só
-- substitui de verdade quando a signature de tipos é idêntica; com tipos
-- novos, o Postgres cria uma SEGUNDA function sobrecarregada (5 parâmetros),
-- deixando a versão antiga sem validação ainda chamável. Precisa dropar a
-- assinatura antiga explicitamente antes de criar a nova.
drop function if exists public.create_order(uuid, public.payment_method, uuid, numeric, jsonb);
